...
/**
 * Build elements + connectors with proper UUIDs.
 * An internal idMap maps short placeholder keys (e.g. "model") → real UUID
 * so connector source_id/target_id references stay consistent.
 */
function makeSeed(
  diagramId: string,
  elInputs: ElementInput[],
  connInputs: ConnectorInput[]
): SeedData {
  // Map placeholder → UUID for element IDs
  const idMap = new Map<string, string>();
  for (const el of elInputs) {
    idMap.set(el.id, uuidv4());
  }

  const elements: DiagramElement[] = elInputs.map((el) => ({
    id: idMap.get(el.id)!,
    ...
    stereotype: '',
    notes: '',
    attributes: el.type === 'class-box' ? [{ visibility: '+' as const, name: 'id', type: 'UUID' }] : [],
    methods: el.type === 'class-box' ? [{ visibility: '+' as const, name: 'execute', returnType: 'void', params: '' }] : [],
  }));

  const connectors: Connector[] = connInputs.map((c) => ({
    id: uuidv4(),
    diagram_id: diagramId,
    source_id: idMap.get(c.source)!,
    target_id: idMap.get(c.target)!,
  ...
  switch (diagram.view_type) {
    ...
    case 'process':
      return makeSeed(diagram.id, [
        ...
        { id: 's1c', source: 'a1', target: 'a2', relation: 'association' },
        { id: 's2c', source: 'a2', target: 'a3', relation: 'association' },
        { id: 's3c', source: 'a3', target: 'a4', relation: 'association' },
        { id: 's4c', source: 'a4', target: 's1', relation: 'association' },
      ]);
    case 'physical':
      return makeSeed(diagram.id, [
